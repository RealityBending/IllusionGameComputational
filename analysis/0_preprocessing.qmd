...
illu |> 
  summarize(ISI = mean(ISI), .by = c("Dataset", "Illusion_Type"))
# TODO: Sanitize ISI (directly in the original preprocesing scripts)
...
# TODO: remove the filtering once ISI is consistent
m_isi1 <- mgcv::bam(RT ~ Group + s(ISI, k = -1, by = `Group`, bs = "cr"),
                   data = mutate(illu, Group = as.factor(paste0(Dataset, "_", Illusion_Type))) |> 
                     filter(Dataset %in% c("IllusionGameValidation", "IllusionGameEEG", "IllusionGameReliability")))
