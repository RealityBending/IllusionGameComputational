...
fit_model <- function(f, data, name = "gam_lnr_muller") {
  ...
  params <- unique(priors$dpar)
  for (par in c("", params[!params %in% c("poutlier")])) {
    priors <- c(priors,
      brms::prior_string("normal(0, 1)", class = "b", dpar = par),
      replace = TRUE
    )
  }
...
#
# # TODO: WAIC instead of loo will probably be faster.
# # m1_lnr_muller <- brms::add_criterion(m1_lnr_muller, "loo", moment_match = FALSE)
# # m1_lnr_muller <- brms::add_criterion(m1_lnr_muller, "waic")
#
#
